...
class TestEdgeFlagsInit:
    ...
    async def test_init_failure_raises(self, httpx_mock: HTTPXMock) -> None:
    ...
    async def test_init_failure_uses_bootstrap(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(status_code=500)
        ready_called: list[bool] = []
        client = EdgeFlags(
            "tok",
...
class TestEdgeFlagsMethods:
    async def test_flag_and_config(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
    ...
    async def test_flag_default(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
    ...
    async def test_all_flags_and_configs(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
...
class TestEdgeFlagsChangeEvents:
    async def test_refresh_emits_change(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
        ...
        assert change["flags"][0]["previous"] is True
        assert change["flags"][0]["current"] is False
        client.destroy()

    async def test_refresh_no_change(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
...
class TestEdgeFlagsIdentify:
    async def test_identify_refreshes(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
        ...
        await client.identify({"user_id": "u1", "custom": {}})

        assert client.flag("beta") is True
        client.destroy()


class TestEdgeFlagsDestroy:
    async def test_destroy_clears_state(self, httpx_mock: HTTPXMock) -> None:
        httpx_mock.add_response(json=EVAL_RESPONSE)
        client = EdgeFlags("tok", "http://localhost")
        await client.init()
